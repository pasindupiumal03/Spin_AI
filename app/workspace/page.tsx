  ...
  useEffect(() => {
    ...
    if (generationRequest && !existingFiles) {
      ...
      if (storedPromptFileHistory) {
        ...
        // Initialize chatHistory from promptFileHistory
  ...
  const handleChatSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.files) {
        ...
        const changedFilesList: { path: string; status: "updated" | "new" }[] = [];

        Object.keys(newFiles).forEach((filePath) => {
          if (!prevFiles[filePath]) {
            changedFilesList.push({ path: filePath, status: "new" });
          } else if (prevFiles[filePath] !== newFiles[filePath]) {
            changedFilesList.push({ path: filePath, status: "updated" });
          }
        });
        ...
        sessionStorage.setItem("generatedFiles", JSON.stringify(newFiles));
        sessionStorage.setItem("changedFiles", JSON.stringify(changedFilesList));
        setChatPrompt("");
      } else {
